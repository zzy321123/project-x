...
__declspec(dllexport) Util::CMemLeakDetect g_MemLeakDetect;

namespace Util
{
	CMemLeakDetect::CMemLeakDetect() : m_pMallocMap(0),m_pOldHook(0),m_hProcess(0),m_pLogFile(0)
	{
		...
		Char sHeader[128] = {0};
		sprintf(sHeader,"====================Log(CreateTime : %04d-%02d-%02d %02d:%02d:%02d)====================\r\n\r\nMemLeakDetect Installed.\r\n",
			xTM.tm_year + 1900,
			xTM.tm_mon  + 1,
			xTM.tm_mday,
			xTM.tm_hour,
	...
	Int32 CMemLeakDetect::AllocHook(Int32 iType, void* pData, Size_t iSize, Int32 iUse, Long iRequest, const UChar* szFile, Int32 iLine)
	{
		static Bool bInallocHook = false;

		if (bInallocHook || (iUse == _CRT_BLOCK)) 
		{
			if (g_MemLeakDetect.m_pOldHook)
				return g_MemLeakDetect.m_pOldHook(iType, pData, iSize, iUse, iRequest, szFile, iLine);
		...
		{
		case _HOOK_ALLOC:
			g_MemLeakDetect.HookMalloc(iRequest);
			break;

		case _HOOK_FREE:
			g_MemLeakDetect.HookFree(pData);
			break;

		case _HOOK_REALLOC:
			g_MemLeakDetect.HookRealloc(pData, iRequest);
			break;
		...
		Int32 iRet = 0;
		if (g_MemLeakDetect.m_pOldHook)
			iRet = g_MemLeakDetect.m_pOldHook(iType, pData, iSize, iUse, iRequest, szFile, iLine);

		bInallocHook = false;
		return iRet;
	}

	void CMemLeakDetect::Report(const Char* pFormat, ...)
	{
		Char sMsg[512] = {0};
		...
		{
			_mkdir("Logs");
			m_pLogFile = fopen("Logs/MemLeak.log","wb");
		}
	...
	Char* CMemLeakDetect::BuildSymbolPath()
	{
		Char*	pEnv	= 0;
		UInt32  iIndex	= 0;
		UInt32  iLength = 0;
	...
	void CMemLeakDetect::GetStackTrace(void* pCallStack)
	{
	...
	void CMemLeakDetect::HookFree(const void* pData)
	{
		BlockMap* pMallocMap = (BlockMap*)m_pMallocMap;
	...
	void CMemLeakDetect::HookMalloc (Int32 iRequest)
	{
		BlockMap* pMallocMap = (BlockMap*)m_pMallocMap;
	...
	void CMemLeakDetect::HookRealloc(const void* pData, Int32 iRequest)
	{
		HookFree(pData);
		HookMalloc(iRequest);
	}	

	void CMemLeakDetect::ReportLeaks ()
	{
		CallStack*	pStack			= 0;
				...
				for (iFrame = 0; iFrame < pStack->size(); iFrame++)
				{
					if ((bFoundLine = SymGetLineFromAddr(m_hProcess, (*pStack)[iFrame], &dwDisplacement, &sSourceInfo)) == TRUE)
					{
						if (strstr(sSourceInfo.FileName, "memleakdetect") ||
							strstr(sSourceInfo.FileName, "malloc")	  ||
