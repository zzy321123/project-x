#ifndef C_XMLELEMENT_H
#define C_XMLELEMENT_H
...
namespace Util
{
	...
	/* XML节点类型封装定义                                                  */
	/************************************************************************/
	template<class StringType> class CXmlElement : public CRefCounter
	{
	public:
		//节点默认构造函数
		CXmlElement(const StringType& sTag = StringType())
		{						
			Level  = 0;
			Parent = 0;			
			Tag    = sTag;
		...
		//节点赋值操作符
		CXmlElement<StringType>& operator = (const CXmlElement<StringType>& sEle)
		{
			if (this != &sEle)
			{
				Clear();
	...
	public:
		//属性类型定义
		typedef CXmlAttribute<StringType> AttributeType;

		//节点类型定义
		typedef CXmlElement<StringType>	  ElementType;	
	...
	public:
		...
		StringType ToString() const
		{
			...
			CStringUtil::Append<StringType,AString>(sTmp,"<");
			CStringUtil::Append<StringType,StringType>(sTmp,Tag);

			for (Size_t i = 0;i<Attributes.size();i++)
			{
				CStringUtil::Append<StringType,AString>(sTmp," ");
				CStringUtil::Append<StringType,StringType>(sTmp,Attributes[i].Key);
				CStringUtil::Append<StringType,AString>(sTmp," = \"");
				CStringUtil::Append<StringType,StringType>(sTmp,Attributes[i].Value);
				CStringUtil::Append<StringType,AString>(sTmp,"\"");
			}
			...
			else
			{
				...
				if(Value.size()>0)
				{
					...
					CStringUtil::Append<StringType,StringType>(sTmp,Value);
					CStringUtil::Append<StringType,AString>(sTmp,"\r\n");
				}

				for (Size_t i = 0;i<Children.size();i++)
				{
					CStringUtil::Append<StringType,StringType>(sTmp,Children[i].ToString());
				}
	...
	//XML节点不同编码的定义
	typedef CXmlElement<AString>		AXmlElement;
	typedef CXmlElement<UString>		UXmlElement;
	typedef CXmlElement<WString>		WXmlElement;
}
#endif
