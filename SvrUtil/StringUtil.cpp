...
{	
	//Paul Hsieh's Hash算法
	UInt32 CStringUtil::CalcHash(const Char * pData, Size_t iSize)
	{
		if (!pData) return 0;
	...
	//字符到16进制转换
	AString CStringUtil::Char2Hex(Char cCh)
	{
	...
	//格式化Url编码
	AString CStringUtil::UrlEncode(const AString& sUrl)
	{
		AString sEscaped = "";
	...
	//转换为UTF8编码
	AString CStringUtil::CodePointToUTF8(UInt32 iCp)
	{
		AString sResult;
	...
	//ascii转unicode编码
	UInt32 CStringUtil::AsciiToUnicode(const Char* pSrc, UInt32 iSrcLen, WChar* pDst, UInt32 iDstLen)
	{
		if(!pSrc || !iSrcLen || !pDst || !iDstLen)
			return 0;

		iconv_t hConv = iconv_open((const char *)"UTF-16LE", (const char *)"GB18030");
		if (hConv == (iconv_t)UTIL_ERROR) 
			return 0;
		...
		if (iconv(hConv, &pInBuf, &iInSize, &pOutBuf, &iOutSize) == (Size_t)UTIL_ERROR)
		{
			iconv_close(hConv);
			return 0;
		}
	...
	//unicode转ascii编码
	UInt32 CStringUtil::UnicodeToAscii(const WChar* pSrc,UInt32 iSrcLen,Char* pDst, UInt32 iDstLen)
	{
		if(!pSrc || !iSrcLen || !pDst || !iDstLen)
			return 0;
	...
	//utf8转unicode编码
	UInt32 CStringUtil::Utf8ToUnicode(const Utf8* pSrc,UInt32 iSrcLen,WChar* pDst,UInt32 iDstLen)
	{
		if(!pSrc || !iSrcLen || !pDst || !iDstLen)
			return 0;
	...
	//unicode转utf8编码
	UInt32 CStringUtil::UnicodeToUtf8(const WChar* pSrc,UInt32 iSrcLen,Utf8* pDst,UInt32 iDstLen)
	{
		if(!pSrc || !iSrcLen || !pDst || !iDstLen)
			return 0;
	...
	//ascii转utf8编码
	UInt32 CStringUtil::AsciiToUtf8(const Char* pSrc,UInt32 iSrcLen,Utf8* pDst,UInt32 iDstLen)
	{
		if(!pSrc || !iSrcLen || !pDst || !iDstLen)
			return 0;
	...
	//utf8转ascii编码
	UInt32 CStringUtil::Utf8ToAscii(const Utf8* pSrc, UInt32 iSrcLen, Char* pDst, UInt32 iDstLen)
	{
		if(!pSrc || !iSrcLen || !pDst || !iDstLen)
			return 0;
	...
	//////////////////////////////////////////////////////////////////////////
	//stl字符串编码转换
	WString CStringUtil::ToWString(const AString& sVal)
	{
		WString sRet;
	...
	WString	CStringUtil::ToWString(const UString& sVal)
	{
		WString sRet;
	...
	AString CStringUtil::ToString(const WString& sVal)
	{
		AString sRet;
	...
	AString	CStringUtil::ToString(const UString& sVal)
	{
		AString sRet;
	...
	UString CStringUtil::ToUtf8(const WString& sVal)
	{
		UString sRet;
	...
	UString	CStringUtil::ToUtf8(const AString& sVal)
	{
		UString sRet;
	...
	UString  _Utf8(const Char* sVal)
	{
		...
		return CStringUtil::ToUtf8(sVal);
	}
	...
	AString  _Ascii(const UChar* sVal)
	{
		...
		return CStringUtil::ToString(sVal);
	}
	...
	WString  _Unicode(const Char* sVal)
	{
		...
		return CStringUtil::ToWString(sVal);
	}
