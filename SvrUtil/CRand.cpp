...
namespace Util
{
	...
	Int32 CRand::RandInt(Int32 iMax)
	{
		return RandInt() % (iMax + 1);
	}

	Int32 CRand::RandInt(Int32 iLow, Int32 iHigh)
	{
		A_Exception(iHigh >= iLow && "Random Range Error.");
		return RandInt(iHigh - iLow) + iLow;
	}
	
	Float CRand::RandFloat()
	{
		return (Float)RandInt() / (Float)M;
	}

	Float CRand::RandFloat(Float fMax)
	{
		return RandFloat() * fMax;
	}

	Float CRand::RandFloat(Float fLow, Float fHigh)
	{
