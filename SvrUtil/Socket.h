...
namespace Util
{
	...
	/* Socket封装                                                           */
	/************************************************************************/
	class UTIL_API CSocket : public CRefCounter
	{		
	public:		
		//构造
		CSocket(SOCKET hSocket = INVALID_SOCKET);
		
		//析构
		virtual ~CSocket();

		//赋值操作符
		CSocket& operator = (const CSocket& sSocket);
	...
	public:
		...
		//接受连接
		virtual Bool	Accept(CSocket& sSocket, SocketAddr& sAddr);
